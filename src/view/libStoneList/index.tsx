...
import * as API from '../../API/stone'
import StoneRTK from '../../store/stone'
import { StoneState } from '../../store/stone/types'
import { NavPropsLibStone } from '../../routes/types'
...
const LibStoneList: React.FC<NavPropsLibStone> = ({ navigation }) => {
  ...
  const { data: response, isFetching } = useQuery('getStones', API.getAll, {
    staleTime: 60000 * 60 * 24,
    cacheTime: 60000 * 60 * 24,
  })
  if (isFetching && !stone.length) {
    return <FakeLoadingScreen />
  }
  if (response && !('statusCode' in response)) {
    dispatch(StoneRTK.actions.setStone(response))
  }
  const reFetch = () => {
    queryClient.invalidateQueries('getStones')
  }
  ...
  const goDetail = async ({ id }: GoDetailProps) => {
    navigation.navigate('LibStoneDetail', {
      stoneId: id,
    })
  }

  const renderList = (item: StoneState) => (
